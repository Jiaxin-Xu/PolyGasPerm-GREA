...
def build_selection_dataset(args, model, dataset):
    print('begin pseudo labels sampling')
    unlabeled_set = dataset[dataset.get_unlabeled_idx()]
    labeled_set = dataset[dataset.get_labeled_idx()]
    # labeled_set = dataset[dataset.get_labeled_idx()[args.train_index]]
        ...
        if batch.x.shape[0] != 1:
            with torch.no_grad():
                ...
                if args.model == 'grea':
                    ...
                    output = model(batch)
                    pred, h_graph = output[0], output[1]
                    h_graph = torch.repeat_interleave(h_graph, batch.batch[-1]+1, dim=0)
                    noises = torch.normal(0, 1, size=(h_graph.size(0), args.emb_dim)).to(args.device)
                    h_graph[:,:args.emb_dim] = h_graph[:,:args.emb_dim] + noises
                    y_noised = model.predictor(h_graph)
                    var = y_noised.view(-1).view(-1, batch.batch[-1]+1).var(dim=1)
                unlabeled_env_vars.append(var.detach())                         
                if args.pred_log:
                    unlabeled_pred.append(10**pred.detach())
                else:
                    unlabeled_pred.append(pred.detach())
    unlabeled_pred = torch.cat(unlabeled_pred, dim = 0).cpu().view(-1)
    if args.pred_log:
        unlabeled_pred = torch.clamp(unlabeled_pred, max=10000)
    unlabeled_env_vars = torch.cat(unlabeled_env_vars, dim = 0).cpu().view(-1)
    labeled_true = dataset.data.y[dataset.get_labeled_idx()].cpu().view(-1)
    # labeled_true = dataset.data.y[dataset.get_labeled_idx()[args.train_index]].cpu().view(-1)
        ...
        if batch_labeled.x.shape[0] != 1:
            with torch.no_grad():
                if args.model == 'grea':
                    ...
                    pred, h_graph = output[0], output[1]
                    h_graph = torch.repeat_interleave(h_graph, batch_labeled.batch[-1]+1, dim=0)
                    noises = torch.normal(0, 1, size=(h_graph.size(0), args.emb_dim)).to(args.device)
                    h_graph[:,:args.emb_dim] = h_graph[:,:args.emb_dim] + noises
                    y_noised = model.predictor(h_graph)
                    var = y_noised.view(-1).view(-1, batch_labeled.batch[-1]+1).var(dim=1)
                labeled_env_vars.append(var.detach())
    labeled_env_vars = torch.cat(labeled_env_vars, dim = 0).cpu().view(-1)
    uncertainty_masks = unlabeled_env_vars <= torch.quantile(labeled_env_vars, 0.5)
    unlabeled_pred = unlabeled_pred[uncertainty_masks]
    unlabel_idx = unlabel_idx[uncertainty_masks]
    start = labeled_true.min()
    end = labeled_true.max()
    anchor_num = 4
    ...
    for idx, anchor in enumerate(bucket_center):
        ...
        num_picked = (valid_mask.sum() * torch.pow(sampling_probs[idx], 1 / 1)).to(torch.int).item()
    ...
    new_trainloader = DataLoader(
        ...
        shuffle=True, 
        num_workers = 0)
    print('done pseudo labels sampling')
    return new_trainloader



def build_augment_dataset(args, model, dataset, including_new=None):
    print('begin label-anchored mixup')
    labeled_set = dataset[dataset.get_labeled_idx()]
    # labeled_set = dataset[dataset.get_labeled_idx()[args.train_index]]
    unlabeled_set = dataset[dataset.get_unlabeled_idx()]
    if including_new is not None:
        labeled_trainloader = including_new
    else:
        labeled_trainloader = DataLoader(
            labeled_set, 
            batch_size=args.batch_size, 
        ...
        if batch.x.shape[0] != 1:
            with torch.no_grad():
                if args.model == 'grea':
                    ...
                    output = model(batch)
                    labeled_p, labeled_h = output[0], output[1]
    ...
    cenRep_labels, cenRep_reps = torch.clone(labeled_true), torch.clone(labeled_reps)
        ...
        if batch.x.shape[0] != 1:
            with torch.no_grad():
                ...
                if args.model == 'grea':
                    ...
                    output = model(batch)
                    unlabeled_p, unlabeled_h = output[0], output[1] 
            # unlabeled_pred.append(torch.pow(10, unlabeled_p.detach())) 
            if args.pred_log:
                unlabeled_pred.append(10**unlabeled_p.detach())
            else:
    ...
    cenRep_reps = torch.cat([cenRep_reps, unlabeled_reps], dim=0)
    cenRep_labels = torch.cat([cenRep_labels, unlabeled_pred], dim=0)
    ...
    bucket_id_per_label = torch.bucketize(cenRep_labels.view(-1), boundaries)
    bucket_id_per_label = torch.clamp(bucket_id_per_label, min=1, max=len(boundaries)-1)
    ...
    def mean_by_groups(sample_rep, groups):
        ...
        index = torch.arange(mean.shape[0])[group_count > 0]
    ...
    print('done label-anchored mixup')
