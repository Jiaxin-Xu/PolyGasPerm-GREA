...
class PolymerRegDataset_OnlyTest(InMemoryDataset):
    def __init__(self, name='plym-oxygen', root ='data', transform=None, pre_transform = None, use_valid_file_name='_validv5.csv'):
        ...
        self.dir_name = '_'.join(name.split('-')) + '_test'
        ...
        super(PolymerRegDataset_OnlyTest, self).__init__(self.processed_root, transform, pre_transform)
    ...
    def process(self):
        data_list = read_graph_list(osp.join(self.path_abs,'test_raw.csv'), property_name=self.name, process_labeled=True)
